...
update_config "$kafka_config" 'advertised.listeners' "PLAINTEXT://$KAFKA_ADDRESS:9092,SSL://$KAFKA_ADDRESS:9093"
update_config "$kafka_config" 'advertised.host.name' "$KAFKA_ADDRESS"
update_config "$kafka_config" 'advertised.port' "9093"
update_config "$kafka_config" 'listeners' "PLAINTEXT://:9092,SSL://:9093"
update_config "$kafka_config" 'ssl.client.auth' "required"
update_config "$kafka_config" 'zookeeper.connect' "localhost:$ZOOKEEPER_PORT"
######## SSL ############
update_config "$kafka_config" 'ssl.truststore.location' "/certificates/kafka.truststore"
update_config "$kafka_config" 'ssl.truststore.password' "111111"
update_config "$kafka_config" 'ssl.keystore.location' "/certificates/kafka.keystore"
update_config "$kafka_config" 'ssl.keystore.password' "111111"
update_config "$kafka_config" 'ssl.key.password' "111111"
update_config "$kafka_config" 'ssl.keystore.type' "JKS"
update_config "$kafka_config" 'ssl.truststore.type' "JKS"
update_config "$kafka_config" 'ssl.enabled.protocols' "TLSv1.2,TLSv1.1,TLSv1"
update_config "$kafka_config" 'ssl.endpoint.identification.algorithm' ""
update_config "$kafka_config" 'listener.security.protocol.map' "PLAINTEXT:PLAINTEXT,SSL:SSL"
